...
namespace NewSR2MP
{
    public static class Globals
    {
        ...
        {
            ammoPointersToPlotIDs.TryAdd(storage.Ammo.Pointer,
                storage.GetComponentInParent<LandPlotLocation>()._id + storage.AmmoSetReference.name);

            if (!ammoByPlotID.TryAdd(storage.GetComponentInParent<LandPlotLocation>()._id + storage.AmmoSetReference.name, storage.Ammo))
                ammoByPlotID[storage.GetComponentInParent<LandPlotLocation>()._id + storage.AmmoSetReference.name] = storage.Ammo;
        }
